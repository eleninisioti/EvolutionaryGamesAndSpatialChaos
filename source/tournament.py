...
class Tournament:
  ...
  def __init__(self, args):
    ...
    self.cost = args.cost
    self.game = args.game
    self.nagents = args.nagents  # square grid
    ...
    self.counter = 0
    self.day_duration = args.day_duration
    self.night_duration = args.night_duration
    self.prob_move = args.prob_move
  ...
  def create_agents(self, init_coop, grid_length):
    ...
    for index, i in enumerate(agent_positions):
      ...
      agent = Agent(nagents=self.nagents, idx=index, location=location, init_strategy=init_strategy, radius=self.radius)
  ...
  def move_night(self):
    #agents_remaining = copy.deepcopy(self.agents)
    random.shuffle(self.agents)
    self.save_locs = [agent.location for agent in self.agents]
    square_width = int(np.ceil(np.sqrt(self.nagents))/2)
    count = 0
    center_x = center_y = int(np.ceil(int(self.grid_length/2)))
    exists = False
    keep_idxs = []
    for hor in range(center_x-square_width, center_x + square_width + 1):
      for ver in range(center_y - square_width, center_y + square_width + 1):
        if count < len(self.agents):
          if hor >= 0 and hor<self.grid_length:
            if ver >= 0 and ver < self.grid_length:
              #print('agent was in ', self.agents[count].location)
              self.agents[count].location = [hor,ver]
              #self.agents[count].find_neighbors(self.radius)
              print(self.agents[count].neighbors)

              count += 1
    for idx in range(len(self.agents)):
      self.agents[idx].find_neighbors(self.radius)
      #(self.agents[idx].neighbors)
      #print(self.agents[idx].location)
  ...
  def move(self):
    if self.counter < self.day_duration:
      #  day starts
      print("moving day")
      self.move_day(prob_move=self.prob_move)

    if self.counter == self.day_duration:
      #  night starts
      if self.night_duration > 0:
        print("moving night")
        self.move_night()

    if (self.counter == self.day_duration + self.night_duration - 1):
      # new day
      print("new day")
      self.counter = -1
      if self.night_duration:
        for idx in range(len(self.agents)):
          self.agents[idx].location = self.save_locs[idx]
      # if self.night_duration > 0:
      #
      #   self.move_day(prob_move=1, replace=False)
